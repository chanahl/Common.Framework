...
namespace Common.Framework.Bots.Bots
{
    public abstract class OutlookService : Bot
    {
        ...
        public void Send()
        {
            try
            {
                ...
                ConsoleUtilities.WriteToConsole(message, Core.Constants.DateTimeFormatIso8601);
                LogManager.Instance().LogInfoMessage(message);
            }
            catch (SystemException systemException)
            {
                var systemExceptionMessage = "Exception: " + systemException.Message;
                ConsoleUtilities.WriteToConsole(systemExceptionMessage, Core.Constants.DateTimeFormatIso8601);
